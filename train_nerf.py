...
if __name__ == '__main__':
    ...
    parser.add_argument('--curvature_loss', '--C', action='store_true',
                        help="use curvature loss term, slower but make surface smoother")
    ...
    if opt.network in ['tcnn', 'enc', 'sdf', 'phasor']:
        optimizer = lambda model: torch.optim.Adam([
            ...
            {'name': 'net', 'params': list(model.sdf_net.parameters()) + list(model.color_net.parameters()) + list(
                model.deviation_net.parameters()), 'weight_decay': 1e-6},
