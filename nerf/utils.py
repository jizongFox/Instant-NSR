...
def get_rays(c2w, intrinsics, H, W, N_rays=-1):
    ...
    i, j = torch.meshgrid(torch.linspace(0, W - 1, W, device=device),
                          torch.linspace(0, H - 1, H, device=device))  # for torch < 1.10, should remove indexing='ij'
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, staged=False, bg_color=bg_color, perturb=True,
                                    cos_anneal_ratio=min(self.epoch / 200, 1.0),
                                    normal_epsilon_ratio=min(self.epoch / 200, 0.95),
                                    **self.conf)
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, staged=True, bg_color=bg_color, perturb=False,
                                    cos_anneal_ratio=min(self.epoch / 100, 1.0),
                                    normal_epsilon_ratio=min((self.epoch - 50) / 100, 0.99),
                                    **self.conf)
    ...
    def save_mesh(self, save_path=None, resolution=256, aabb=None, bound=1, threshold=0., use_sdf=False):
        ...
        vertices, triangles = extract_geometry(bounds_min, bounds_max, resolution=resolution, threshold=threshold,
                                               query_func=query_func, use_sdf=use_sdf)
    ...
    def test(self, loader, save_path=None):
        ...
        for i, data in enumerate(loader):
            ...
            cv2.imwrite(path_normal, cv2.cvtColor((preds_normal[0].detach().cpu().numpy() * 255).astype(np.uint8),
                                                  cv2.COLOR_RGB2BGR))
        ...
        for data in loader:
            ...
            if self.local_rank == 0:
                ...
                if self.use_tensorboardX:
                    ...
                    self.writer.add_scalar("train/inv_s", 1.0 / self.model.forward_variance()[0, 0].detach(),
                                           self.global_step)
                    self.writer.add_scalar("train/lr", self.optimizer.param_groups[0]['lr'], self.global_step)

                if self.scheduler_update_every_step:
                    pbar.set_description(
                        f"loss={loss.item():.4f} ({total_loss / self.local_step:.4f}), s_val={self.model.forward_variance()[0, 0].detach().cpu():.2f}, lr={self.optimizer.param_groups[0]['lr']:.6f}")
                else:
                    pbar.set_description(
                        f"loss={loss.item():.4f} ({total_loss / self.local_step:.4f}), s_val={self.model.forward_variance()[0, 0].detach().cpu():.2f}, lr={self.optimizer.param_groups[0]['lr']:.6f}")
                pbar.update(1)
        ...
        if self.local_rank == 0:
            ...
            if self.use_tensorboardX:
                ...
                cv2.imwrite(save_path_gt,
                            cv2.cvtColor((truths[0].detach().cpu().numpy() * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
