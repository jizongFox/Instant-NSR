...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, num_steps, bound, upsample_steps, bg_color, cos_anneal_ratio=1.0,
            normal_epsilon_ratio=1.0):
        ...
        if upsample_steps > 0:
            with torch.no_grad():
                ...
                for i in range(upsample_steps // 16):
                    ...
                    z_vals, sdf = self.cat_z_vals(rays_o, rays_d, z_vals, new_z_vals, sdf, bound,
                                                  last=(i + 1 == upsample_steps // 16))
        ...
        new_pts = rays_o.unsqueeze(-2) + rays_d.unsqueeze(-2) * z_vals_mid.unsqueeze(
            -1)  # [N, 1, 3] * [N, t, 3] -> [N, t, 3]
        ...
        color = self.forward_color(new_pts.reshape(-1, 3), new_dirs.reshape(-1, 3), normal.reshape(-1, 3),
                                   feature_vector, bound)
        ...
        weights = alpha * torch.cumprod(torch.cat([torch.ones([N, 1], device=alpha.device), 1. - alpha + 1e-7], -1),
                                        -1)[:, :-1]
        ...
        if self.curvature_loss:
            ...
            perturbed_pts = new_pts.reshape(-1, 3) + torch.cross(normal, random_vec_norm) * 0.01 * (
                        1.0 - normal_epsilon_ratio)  # naively set perturbed points,
            perturbed_gradient = self.gradient(perturbed_pts.reshape(-1, 3), bound,
                                               0.005 * (1.0 - normal_epsilon_ratio)).squeeze()
            perturbed_normal = perturbed_gradient / (
                        1e-5 + torch.linalg.norm(perturbed_gradient, ord=2, dim=-1, keepdim=True))

            curvature_error = (torch.sum(normal * perturbed_normal, dim=-1) - 1.0) ** 2
            curvature_error = (relax_inside_sphere * curvature_error.reshape(N, num_steps)).sum() / (
    ...
    def run_cuda(self, rays_o, rays_d, num_steps, bound, upsample_steps, bg_color, cos_anneal_ratio,
                 normal_epsilon_ratio):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, bound, self.density_grid,
                                                                    self.mean_density, self.iter_density, counter,
                                                                    self.mean_count, self.training, 128, False)
            ...
            gradient = self.finite_difference_normals_approximator(xyzs, bound,
                                                                   0.005 * (1.0 - normal_epsilon_ratio)).float()  #
            normal = gradient / (1e-5 + torch.linalg.norm(gradient, ord=2, dim=-1, keepdim=True))
            ...
            while step < 1024:  # max step
                ...
                xyzs, dirs, deltas = raymarching.march_rays(n_alive, n_step, rays_alive[i % 2], rays_t[i % 2], rays_o,
                                                            rays_d, bound, self.density_grid, self.mean_density, near,
                                                            far, 128)
                ...
                raymarching.composite_rays(n_alive, n_step, rays_alive[i % 2], rays_t[i % 2], alpha, rgbs, normal,
                                           deltas, weights_sum, depth, image, normal_map)
    ...
    def update_extra_state(self, bound, decay=0.95):
            ...
            for xi, xs in enumerate(X):
                for yi, ys in enumerate(Y):
                    for zi, zs in enumerate(Z):
                        ...
                        density[~mask] = inv_s * torch.exp(inv_s * sdf[~mask]) / (1 + torch.exp(inv_s * sdf[~mask]))
                        tmp_grid[xi * 128: xi * 128 + lx, yi * 128: yi * 128 + ly,
                        zi * 128: zi * 128 + lz] = density.reshape(lx, ly, lz)
        ...
        print(
            f'[density grid] min={self.density_grid.min().item():.4f}, max={self.density_grid.max().item():.4f}, mean={self.mean_density:.4f} | [step counter] mean={self.mean_count} | [SDF] inv_s={inv_s:.4f}')

    def render(self, rays_o, rays_d, num_steps, bound, upsample_steps, staged=False, max_ray_batch=4096, bg_color=None,
               cos_anneal_ratio=1.0, normal_epsilon_ratio=1.0, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    depth_, image_, normal_, gradient_error_, curvature_error_ = _run(rays_o[b:b + 1, head:tail],
                                                                                      rays_d[b:b + 1, head:tail],
                                                                                      num_steps, bound, upsample_steps,
                                                                                      bg_color,
                                                                                      cos_anneal_ratio=cos_anneal_ratio,
                                                                                      normal_epsilon_ratio=normal_epsilon_ratio)
            ...
            depth, image, normal, gradient_error, curvature_error = _run(rays_o, rays_d, num_steps, bound,
                                                                         upsample_steps, bg_color, cos_anneal_ratio,
                                                                         normal_epsilon_ratio)
